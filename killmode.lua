...
local Window = Rayfield:CreateWindow({
   Name = "Gosling Hub | Jujutsu Chronicles V4.5 | " .. getexploit(),
...
local function genocide()
	coroutine.wrap(function()
		if getgenv().genocide == true then
			...
			wait(0.1)
				pcall(function()
					for i,b in pairs(game:GetService("Players"):GetChildren()) do
						if b.Name ~= tostring(game.Players.LocalPlayer) then
							local args_one = {
								[1] = {
								["Victim"] = b.Character,
								["Character"] = game:GetService("Players").LocalPlayer.Character,
								["Action"] = "CutsceneSkill",
								["MiscData"] = game:GetService("Players").LocalPlayer.UIStats,
								["AnimTime"] = 5.916666507720947,
								["Skill"] = "BerserkCombo"
								}
							}
							local args_two = {
								[1] = {
									...
									["Combo"] = 5,
									["Target"] = b.Character,
									["Finisher"] = "GojoPalmCombo",
									["GripCF"] = CFrame.new(252.3905029296875, 12.33823299407959, 237.08718872070312) * CFrame.Angles(3.141592502593994, -0.3933708667755127, 3.141592502593994)
								}
							}
							local args_three = {
								[1] = {
								...
								if v:IsA("Model") and v.Name == b.Name then
									if v.Humanoid.Health > 0 then
										repeat
											wait()
											game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * CFrame.new(0,5,0)
											game:GetService("ReplicatedStorage").Events.CombatEvent:FireServer(unpack(args_one))
											game:GetService("ReplicatedStorage").Events.CombatEvent:FireServer(unpack(args_two))
											game:GetService("ReplicatedStorage").Events.CombatEvent:FireServer(unpack(args_three))
...
local PlayerFarmToggle = MiscTab:CreateToggle({
   Name = "troll mode (use an alt lol)",
   CurrentValue = false,
   Flag = nil,
   Callback = function(Value)
		if Value == true then
